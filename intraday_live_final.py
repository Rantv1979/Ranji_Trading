...
- Signal auto-refresh 10s, Chart auto-refresh 5s
- Nifty 50 / Next 50 / Nifty 100 / Nifty 500 universes
- ₹5,00,000 paper trading account
"""
...
NIFTY_500 = NIFTY_100 + ["TATAMOTORS.NS","ONGC.NS","COALINDIA.NS","ADANIPORTS.NS","HAVELLS.NS","BAJAJ-AUTO.NS","TATASTEEL.NS"]
...
# ------------------ UI ------------------
def main():
    st.title("📊 Intraday Trading Terminal — Live Mode")
    ...
    with tabs[0]:
        ...
        st.metric("Paper Capital", f"₹{paper.cash:,.0f}")
        st.caption("Auto-refresh: Signals 10s • Charts 5s • Paper ₹5 L")
    ...
    with tabs[1]:
        ...
        if market_open():
            for s in syms:
                df=fetch_ohlc(s)
                sg=signal(df,s)
                if sg and sg["conf"]>=conf_min: sigs.append(sg)
        if sigs:
            ...
            st.info("No signals yet (check during live market hours).")

    # --- Charts Tab ---
    with tabs[2]:
        st_autorefresh(interval=CHART_REFRESH_MS,key="chart_refresh")
        sym=st.selectbox("Select Symbol for Chart",NIFTY_50)
        df=fetch_ohlc(sym)
    ...
    # --- Paper Trading Tab ---
    with tabs[3]:
        st.metric("Available Cash",f"₹{paper.cash:,.0f}")
    ...
    # --- Backtest Tab ---
    with tabs[4]:
        st.info("Backtest module can be added later for multi-day simulation. Live mode currently active.")
